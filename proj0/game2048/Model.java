...
public class Model extends Observable {
    ...
    public boolean tilt(Side side) {
        ...
        // set the side to make the operation more convenient
       if (side != Side.NORTH){
           board.setViewingPerspective(side);
       }
       //press up, and turn all tiles be adjacent
        int size = board.size();
    ...
    public static boolean emptySpaceExists(Board b) {
        ...
        int size = b.size(); /** 先提取size */
        for (int i = 0; i < size; i++){
            for (int j = 0; j<size; j++){
                /** 设置读取在i和j index */
                if(b.tile(i,j) == null){ /** 在这某个坐标如果没有数 return true */
                    return true;
                }
            }
        }
